          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-green-50">
                {headers.map((h, hi) => (
                  <th key={hi} className="border border-gray-200 px-4 py-2 text-left font-semibold text-gray-700">
                    {h}
                  </th>
                ))}
              </tr>
...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 sm:px-6 py-14">
      ...
      <p className="text-xs text-gray-400 mb-6">{formatDate(post.date)} · {post.readMin} min lexim</p>
            ...
            {related.map((r) => (
              ...
              >
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <p className="text-xs font-bold text-gray-800 group-hover:text-green-700 transition-colors leading-snug line-clamp-2">
                    {r.title}
                  </p>
                </div>
              </Link>
